...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--output',type=Path,required=True);ap.add_argument('--checkpoint',type=Path,required=True);ap.add_argument('--ncnn-weights',type=Path,required=True);args=ap.parse_args()
    ...
    x=torch.rand(1,3,256,256);wrapped=X2().eval()
    with torch.no_grad():
        ref=wrapped(x);bicubic=torch.nn.functional.interpolate(net(x),scale_factor=.5,mode='bicubic',align_corners=False)
    ...
    package=out/'realesr-animevideov3-x2-tile256-fp16.mlpackage';assert not package.exists()
    model.short_description='Real-ESRGAN animevideov3, NCNN-compatible bicubic 4x to 2x; fixed tile256.'
    ...
    diff=np.abs(pred-ref.numpy());assert pred.shape==(1,3,512,512) and np.isfinite(pred).all()
    assert diff.mean()<.002 and np.quantile(diff,.99)<.01
    result={'status':'pass','checkpointSHA256':sha(checkpoint),'ncnnWeightsSHA256':sha(binary),'officialSourceSHA256':sha(source),'verifiedWeightTensors':len(checks),'ncnnWeightMatch':'all convolution weights match fp16 truncation/denormal flush of official checkpoint; fp32 biases and slopes exact; Core ML uses exact NCNN-stored convolution weights','resize':'separable bicubic A=-0.75 half-pixel centers, replicate edges, no antialias, before clipping; matches installed x2 NCNN graph','resizeMaxError':resize_error,'conversionSeconds':convert_seconds,'firstPredictionSeconds':elapsed,'coremlTorchParity':{'meanAbsoluteError':float(diff.mean()),'p99':float(np.quantile(diff,.99)),'max':float(diff.max())},'package':str(package),'input':[1,3,256,256],'output':[1,3,512,512],'files':[{'path':str(p.relative_to(package)),'bytes':p.stat().st_size,'sha256':sha(p)} for p in sorted(package.rglob('*')) if p.is_file()]}
